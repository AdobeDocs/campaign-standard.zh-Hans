...
* **[!UICONTROL List]**：表示受众是固定的轮廓列表。 这些列表在[工作流](../../automating/using/get-started-workflows.md)中创建，在工作流中保存受众时，数据维度是已知的。 例如，定向活动（尤其是&#x200B;**[!UICONTROL Query]**）之后或对从文件导入的数据进行协调之后。
