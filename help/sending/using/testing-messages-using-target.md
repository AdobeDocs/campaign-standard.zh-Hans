...
除了[测试用户档案](../../audiences/using/managing-test-profiles.md)之外，您还可以通过将自己置于某个目标用户档案的位置来测试电子邮件。 这样，您就可以获得用户档案将收到的消息(自定义字段、动态和个性化信息，包括来自工作流的其他数据……)的精确表示形式。
...
1. 配置消息，然后启动&#x200B;**准备**&#x200B;阶段。
1. **在消息所针** 对的用户档案中选择一个或多个配置文件。
1. 将&#x200B;**替换地址**&#x200B;关联到每个用户档案,验证将被发送到该代码。
...
1. 在&#x200B;**[!UICONTROL Profile substitutions]**&#x200B;选项卡中，单击&#x200B;**[!UICONTROL Create element]**&#x200B;按钮以选择用于测试的用户档案。
...
1. 选择要用于测试的用户档案，然后在&#x200B;**[!UICONTROL Address]**&#x200B;字段中输入所需的替代地址，然后单击&#x200B;**[!UICONTROL Confirm]**。 所有针对用户档案的验证都将发送到此电子邮件地址，而不是发送到此用户档案在数据库中定义的电子邮件地址。
...
1. 定义所有用户档案和替换地址后，您可以发送验证来测试消息。 要执行此操作，请单击&#x200B;**[!UICONTROL Test]**&#x200B;按钮，然后选择要执行的测试类型。
   ...
   请注意，如果未将测试用户档案添加到消息目标，则&#x200B;**[!UICONTROL Email rendering]**&#x200B;和&#x200B;**[!UICONTROL Proof + Email rendering]**&#x200B;选项将不可用。  有关验证发送的详细信息，请参阅[此部分](../../sending/using/sending-proofs.md)。
...
此外，如果受众已配置了地址列和前缀列，您将能够在&#x200B;**[!UICONTROL Profile substitutions]**&#x200B;选项卡中导入这些信息。 [本节](#use-case)中详细介绍了具有替换地址的受众导入示例。
...
1. 单击 **[!UICONTROL Import]** 按钮。与消息目标对应的受众的用户档案被添加到&#x200B;**[!UICONTROL Profile substitution]**&#x200B;选项卡，以及相关的替换地址和前缀。
...
如果要在消息中使用个性化字段，则必须在&#x200B;**启动消息准备之前添加**。 否则，预览中不会考虑这些因素。 因此，如果对个性化字段做了任何更改，请确保再次启动消息准备。
...
1. 构建工作流以从文件创建受众。 为此，请添加和配置以下活动:
   ...
   * **[!UICONTROL Reconciliation]** 活动:将文件中的信息链接到数据库中的信息。在本示例中，我们将用户档案的电子邮件地址用作协调字段(有关此活动的详细信息，请参阅[此部分](../../automating/using/reconciliation.md))。
...
1. 添加&#x200B;**[!UICONTROL Query]**&#x200B;和&#x200B;**[!UICONTROL Email delivery]**&#x200B;活动，然后根据您的需求配置它们(请参阅[查询](../../automating/using/query.md)和[电子邮件投放](../../automating/using/email-delivery.md)部分)。
...
1. 验证根据&#x200B;**[!UICONTROL Profile substitutions]**&#x200B;选项卡中配置的内容发送。

   ![](assets/substitution_uc_proofs.png)

## 教程视频{#video}

此视频显示如何使用用户档案替换测试电子邮件。
