...
此外， [测试用户档案](../../audiences/using/managing-test-profiles.md)，通过将您自己置于某个定向用户档案的位置，可以测试电子邮件。 这样，您就可以获得用户档案将收到的消息的确切呈现形式（自定义字段、动态和个性化信息，包括来自工作流的附加数据……）。
...
1. **预览** 在Email Designer中如何显示用户档案的消息。
1. 发送校样。

   >[!IMPORTANT]
   >
   >验证处理方式 [!DNL Campaign Standard] 作为标准投放。 因此，使用用户档案替换发送校样会将记录添加到所选用户档案的投放和跟踪日志中。

![](assets/do-not-localize/how-to-video.png) [在视频中发现此功能](#video)

## 选择用户档案和替换地址 {#selecting-profiles}

要使用目标用户档案进行测试，您必须首先选择它们，然后定义将接收校样的替换地址。 要执行此操作，您可以 [选择特定配置文件](#selecting-individual-profiles) 在目标用户档案中，或者 [从现有受众导入用户档案](#importing-from-audience).
...
1. 选择要用于测试的配置文件，然后在 **[!UICONTROL Address]** 填写所需的替代地址，然后单击 **[!UICONTROL Confirm]**. 定向到用户档案的所有校样都将发送到此电子邮件地址，而不是发送到数据库中为此用户档案定义的电子邮件地址。

   如果要为验证的主题行添加特定前缀，请填写 **[!UICONTROL Subject line prefix]** 字段。
...
1. 用户档案将添加到列表中，并带有与其关联的替代地址和前缀。 对要用于测试的所有配置文件重复上述步骤，然后单击 **[!UICONTROL Confirm]**.
...
>[!IMPORTANT]
>
>如果您对消息进行了任何更改，请确保再次启动消息准备。 否则，更改将不会反映在验证中。

### 从受众导入用户档案 {#importing-from-audience}

Campaign Standard允许您导入可用于测试的用户档案受众。 例如，这可让您将针对不同用户档案的整组消息发送到一个唯一的电子邮件地址。

此外，如果您的受众已配置了地址列和前缀列，则可以将这些信息导入 **[!UICONTROL Profile substitutions]** 选项卡。 有关使用替代地址导入受众的详情，请参见 [本节](#use-case).

>[!NOTE]
>
>导入受众时，只会选择与消息目标对应的用户档案并将其添加到 **[!UICONTROL Profile substitutions]** 选项卡。

要导入用户档案以从受众进行测试，请执行以下步骤：
...
1. 单击 **[!UICONTROL Import]** 按钮。受众中与消息目标对应的用户档案将添加到 **[!UICONTROL Profile substitution]** 制表符以及关联的替换地址和前缀。

![](assets/substitution_audience_imported.png)

>[!NOTE]
>
>如果您再次导入相同的受众，并使用不同的替换地址和/或前缀，则除了上次导入的受众之外，这些用户档案还将添加到列表中。
...
要能够使用目标用户档案预览消息，请确保已将这些用户档案添加到 **[!UICONTROL Profile substitution]** 列表(请参阅 [定义用户档案和替代地址](#selecting-profiles))。

如果要在消息中使用个性化字段，则必须添加这些字段 **早于** 启动消息准备。 否则，在预览中不会考虑这些指标。 因此，如果对个性化字段进行了任何更改，请确保再次启动消息准备。
...
1. 单击 **[!UICONTROL Profile Substitution]** 选项卡，以显示已添加以供测试的替换用户档案。
...
在本使用案例中，我们希望向一组特定用户档案发送个性化电子邮件新闻稿。 在发送新闻稿之前，我们希望使用一些定向的用户档案预览新闻稿，并向外部文件中定义的内部电子邮件地址发送校样。
...
1. 准备要导入的文件以创建受众。 在我们的用例中，它应包含用于验证的替换地址，以及要添加到验证主题行中的前缀。
...
1. 构建工作流以从文件创建受众。 为此，请添加并配置以下活动：
   ...
   * **[!UICONTROL Reconciliation]** 活动：将文件中的信息链接到数据库中的信息。 在此示例中，将用户档案的电子邮件地址用作协调字段(有关此活动的更多信息，请参阅 [本节](../../automating/using/reconciliation.md))。
   * **[!UICONTROL Save audience]** 活动：根据导入的文件创建受众(有关此活动的更多信息，请参阅 [本节](../../automating/using/save-audience.md))。

   ![](assets/substitution_uc2.png)

1. 运行工作流，然后转到 **[!UICONTROL Audiences]** 选项卡，以检查受众是否已使用所需信息创建。

   在此示例中，受众由三个配置文件组成。 每个模板都链接到将接收验证的替换电子邮件地址，该地址带有在验证主题行中使用的前缀。
...
1. 单击 **[!UICONTROL Import]** 按钮。受众中的用户档案及其关联的替换地址和主题行前缀将添加到列表中。
   ...
   >[!NOTE]
   >
   >在本例中，受众的所有用户档案均被 **[!UICONTROL Query]** 活动。 如果其中一个用户档案不是消息目标的一部分，则不会将其添加到列表。
...
1. 单击 **[!UICONTROL Profile Substitution]** 选项卡，以显示之前添加的替换用户档案。
