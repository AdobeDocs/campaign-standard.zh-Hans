...
利用疲劳规则，营销人员可设置全局跨渠道业务规则，以自动从营销策划中排除发送营销信息过于频繁的轮廓。

要实施疲劳规则，您需要定义每个轮廓的最大消息数，并选择应用该规则的时段。 在投放准备期间，会根据已发送给轮廓的消息数量，将符合条件的轮廓从投放中排除。
...
1. 在 **[!UICONTROL General]** 选项卡中，定义每个轮廓最大消息数的计算方法。 您可以选择常量阈值或变量。 您还可以针对轮廓和投放对阈值进行优化。 有关更多信息，请参见[定义阈值](#defining-the-threshold)。
...
在投放准备期间，会根据已发送给轮廓的投放数量，将符合条件的轮廓从投放中排除。 您可以在投放日志中查看疲劳规则的执行结果。 请参阅[查看疲劳结果](#viewing-the-fatigue-results)。
...
* 手机（短信）
* 直邮
* 移动应用程序：利用此渠道，可向轮廓或应用程序订阅者发送推送通知。 如果选择向轮廓发送通知，则通知将与多渠道疲劳规则兼容。

  >[!IMPORTANT]
  >
  >疲劳规则与发送给应用程序订阅者的推送通知不兼容。 如果向应用程序订阅者发送消息，则不会应用疲劳规则。

* 所有渠道：利用此选项可将规则应用于所有渠道。 例如，您可决定在所有渠道上每月最多发送 3 条消息。 如果您上周向某个轮廓发送了 2 封电子邮件，而今天尝试发送推送通知时则会排除该轮廓。

**投放类型**

疲劳规则兼容所有投放类型：一次性投放、定期投放、工作流投放和事务型消息。

**事务型消息**&#x200B;可用于发送定向事件 (rtEvent) 的服务消息以及营销消息（定向轮廓），例如再营销消息。 疲劳规则仅与营销消息（定向轮廓）兼容。 由于事件事务型消息不包含轮廓信息，所以就算使用轮廓扩充了内容，也不兼容疲劳规则。 借助事务型消息中的营销消息支持，您可以&#x200B;**对包括营销事务型消息在内的所有渠道应用疲劳规则**。

## 定义阈值 {#defining-the-threshold}

每个疲劳规则可定义一个阈值，即在指定时间段内可以向一个轮廓发送的消息数量上限。 达到此阈值后，只有在所考虑的时段结束后，才会再进行投放。 通过此流程，可在消息数量超过设置的阈值时，自动从投放中排除轮廓，从而避免过度通信。

阈值可以是常量，也可以是变量。 这意味着在指定的时间段内，阈值可能因轮廓而异，甚至对于同一轮廓也可能有所不同。

**使用固定阈值**

阈值表示在相关时段内可发送给轮廓的消息数量上限。
...
* 选择轮廓字段：每个轮廓的阈值将根据所选字段而有所不同。 例如，如果已使用“Communication frequency”字段扩展了轮廓资源，请单击 **[!UICONTROL Threshold computation formula]** 字段右侧的按钮，然后选择您的字段。 对于每个轮廓，阈值都将采用“Communication frequency”字段的值。

  ![](assets/fatigue21.png)

* 定义公式：单击 **[!UICONTROL Threshold computation formula]** 字段右侧的第二个按钮可定义高级阈值计算公式。 例如，您可以根据轮廓所属的区段，索引授权消息的数量。 这意味着属于“Web”区段的轮廓，可接受的消息数量可能会超过其他轮廓。 使用 **[!UICONTROL Iif (@origin='Web', 5, 3)]** 类型的公式，可为 Web 区段的轮廓授权投放 5 条消息，为其他区段授权投放 3 条消息。

  ![](assets/fatigue14.png)

**优化轮廓和投放的阈值**

默认情况下，所有消息都会考虑阈值计算式。 选中 **[!UICONTROL Refine Threshold on profiles and deliveries]** 方框，可在准备投放时筛选要计数的轮廓和投放。

在下方的示例中，仅计数男性轮廓，并且只计数标签以 **Newsletter** 开头的投放。
...
在投放准备期间，会根据已发送给轮廓的投放数量，将符合条件的轮廓从投放中排除。 要查看疲劳规则执行结果，请单击 **[!UICONTROL Deployment]** 块右下角的按钮。
...
* 使用适用于&#x200B;**所有渠道**&#x200B;的&#x200B;**常量阈值**&#x200B;创建疲劳规则：
  ...
  在上周，您的高级轮廓收到了一封促销电子邮件和一封事务型再营销电子邮件。 您还安排了在下周发送一条短信。 今天，您决定发送一条定向所有轮廓的推送通知。 则高级轮廓将被排除在今天的推送之外，因为已达到他们在 2 周内的消息数量上限。

  ![](assets/fatigue23.png)

* 使用基于&#x200B;**轮廓字段**&#x200B;的&#x200B;**变量阈值**&#x200B;创建疲劳规则：

  您已使用“Communication limit”字段扩展了轮廓资源，从而为每个轮廓定义不同的阈值。 在疲劳规则中，根据该字段定义一个变量阈值，并选择 2 天的滑动周期。 让我们看看这两个轮廓示例：John 的“Communication limit”为 1，David 的“Communication limit”为 2。 两者昨天都已收到了一封新闻通讯电子邮件。 您决定今天再给他们发送一封电子邮件。 只有 David 会收到今天这封电子邮件，因为 John 会被排除在目标之外。

  ![](assets/fatigue24.png)

* 使用&#x200B;**阈值计算公式**&#x200B;创建疲劳规则：

  您希望根据轮廓的年龄来更改阈值。 对于年龄低于 40 岁的轮廓，将限制数量规定为 4；对于年龄大于 40 岁的轮廓，将限制数量规定为 2。 您可以直接在疲劳规则中创建一个公式，以根据轮廓的年龄来计算该阈值，而不是为使用扩展字段为每个轮廓定义此阈值。 在本例中，该公式为 **[!UICONTROL Iif (@age<40, 4, 2)]**。
...
* 创建一个可&#x200B;**优化**&#x200B;轮廓和投放阈值的疲劳规则：

  您已使用“Score”字段扩展了轮廓资源，还使用“Type”字段扩展了投放资源。 您要定义一个常量阈值“3”，但要从计数中排除所有投放类型为“Alert”或“Black Friday”的投放以及得分大于 10 的所有轮廓。 执行该规则时，它将对发送给得分小于 10 分的轮廓的、所有不属于“Alert”或“Black Friday”类型的过去和计划投放进行计数。

  ![](assets/fatigue26.png)

以下是关于使用阈值计算公式之疲劳规则的分步示例。

在此使用案例中，我们希望创建一个分类规则，防止向高级轮廓每周投放 2 条以上的消息，以及向标准轮廓每周投放 2 条消息。

为了识别客户和潜在客户，我们使用 **[!UICONTROL Status]** 字段扩展了轮廓，其中的 0 表示高级轮廓，1 表示标准轮廓。

要创建规则，请应用以下步骤：

1. 创建新的 **Fatigue** 类型分类规则。
1. 在 **[!UICONTROL Threshold]** 部分中，创建一个公式以根据各个轮廓计算阈值。 在 **[!UICONTROL Depends on the recipient]** 字段中选择 **[!UICONTROL Threshold type]** 值，然后单击 **[!UICONTROL Threshold computation formula]** 字段右侧的第二个按钮。
...
1. 然后，在 **[!UICONTROL Available fields]** 部分中选择轮廓的 **Status**。
