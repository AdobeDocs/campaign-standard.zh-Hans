...
此 **投放警报** 功能是一个警报管理系统，通过它，一组用户可自动接收包含其投放执行信息的通知。
...
* A **[!UICONTROL Summary]** 显示符合您定义的标准的投放数量，以及您为每个标准选择的标签/颜色。
* A **[!UICONTROL Details]** 部分，其中列出了为相应仪表板定义的所有投放标准以及每个标准的所有投放。
...
>[!NOTE]
>
>要访问和配置功能板和警报标准，您必须具有管理权限或显示在 **投放主管** 安全组。 标准用户无法在Adobe Campaign界面中访问功能板。 他们只能接收警报通知。 有关Adobe Campaign中的用户和安全性的更多信息，请参阅 [用户类型](../../administration/using/users-management.md) 和 [关于安全组](../../administration/using/managing-groups-and-users.md#about-security-groups).

从Adobe Campaign界面中，您可以：

* 创建和管理投放警报仪表板。 请参阅 [创建投放警报仪表板](#creating-a-delivery-alerting-dashboard).
* 定义和管理每个仪表板的投放警报标准。 例如，您可以基于准备失败的投放或仅低吞吐量的投放构建警报。 请参阅 [关于警报标准](#about-alerting-criteria).
...
* 访问每个仪表板所有已发送警报的历史记录。
  ...
  选择仪表板时，默认情况下将显示该仪表板的最后一次发送警报。 屏幕左侧列出了所有已发送的警报。 单击 **[!UICONTROL History]** 列表以访问相应的警报。
...
* **[!UICONTROL Deliveries with low throughput]**：任何投放开始的持续时间均超过定义的持续时间，并且已处理消息的百分比少于定义的百分比，吞吐量低于定义的值。
* **[!UICONTROL Deliveries in progress]**：在定义的范围内计划的任何投放，使用 **[!UICONTROL In progress]** 状态。

>[!NOTE]
>
>应用于上述标准的所有参数都有默认值。 这些值可在 **[!UICONTROL Criteria parameters]** 投放警报功能板的选项卡。 请参阅 [标准参数](#criteria-parameters).
...
* **[!UICONTROL Indicators to add in alerts]**，即显示在通知中的列 **[!UICONTROL Details]** 与所选标准对应的投放部分。

  ![](assets/delivery-alerting_notification_colums.png)

* **[!UICONTROL Alert type]**：表示在通知摘要中投放标准旁边显示的标签和颜色。

  ![](assets/delivery-alerting_notification_labels.png)

* **[!UICONTROL Criteria frequency]**：如果一次投放符合标准，则在监控期间发送的每个通知中都会重复显示该投放。 否则，对于一次投放，根据警报条件，每天只发送一个警报（第一次发生时）。
...
为此，您需要先扩展 **投放** 并添加一个新筛选器，以便您仅选择满足以下条件 **[!UICONTROL Finished]** 状态。

1. 转到 **Adobe Campaign** > **管理** > **开发** > **自定义资源** 并单击 **[!UICONTROL Create]**.
1. 选择 **[!UICONTROL Extend an existing resource]**，选择 **[!UICONTROL Delivery]** 资源，然后单击 **[!UICONTROL Create]** 以对其进行编辑。
...
1. 编辑新的筛选器定义：在 **[!UICONTROL Filter definition]** 窗口，拖放 **[!UICONTROL Status]** 项并将其放入工作区并选择 **[!UICONTROL Finished]** 作为筛选条件。
...
1. 在 **[!UICONTROL Delivery filter applied by this criterion]** 下拉列表，选择之前创建的过滤器。

   ![](assets/delivery-alerting_cus-filter.png)

   可以使用与默认标准相同的方式定义标准的设置。 请参阅 [关于警报标准](#about-alerting-criteria).
...
在 **[!UICONTROL Criteria parameters]** 选项卡 [投放警报仪表板](#creating-a-delivery-alerting-dashboard)，您可以定义适用于在此功能板中选择的标准的设置。
...
* **[!UICONTROL Minimum time threshold for delivery in 'Start pending' status (in minutes)]**：对于具有的所有投放，都会发送通知 **[!UICONTROL Start pending]** 状态超过该字段中指定的持续时间， **[!UICONTROL Start pending]** 状态表示系统尚未考虑这些消息。
* **[!UICONTROL Minimum time required for the computation of the throughput (in minutes)]**：仅投放开始(使用 **[!UICONTROL In progress]** 超过指定持续时间时，会考虑 **[!UICONTROL Deliveries with low throughput]** 标准。
* **[!UICONTROL Maximum percentage of processed messages for the computation of the throughput]**：仅考虑消息百分比低于指定百分比的投放 **[!UICONTROL Deliveries with low throughput]** 标准。
...
此 **投放警报** 该功能可通过电子邮件和仪表板，使所有参与的Adobe Campaign用户自动获知投放执行状态。

现在，当您收到投放警报通知时，您可以按照以下提示进行操作。

首先，检查投放的 **日志** 选项卡，以查看与投放和验证相关的所有信息。 利用红色和黄色图标，可识别错误或警告。 红色图标表示阻止启动投放的严重错误。
...
* **[!UICONTROL Deliveries with preparation failed]**：在以下情况中，在投放准备期间可能会出错：
  ...
  有关此内容的更多信息，请参阅 [准备发送](../../sending/using/preparing-the-send.md). 但是，这些错误通常会在消息分析期间发现。 请参阅 [控制规则](../../sending/using/control-rules.md).
...
* 出现以下问题的可能原因： **[!UICONTROL Delivery with bad error ratio for soft bounces]** 警报可以是：
  ...
  欲了解更多信息，请查看 **[!UICONTROL Exclusion logs]** 和 **[!UICONTROL Exclusion causes]** 投放日志的选项卡。 请参阅 [排除日志](../../sending/using/monitoring-a-delivery.md#exclusion-logs).
   ...
   * 一般而言，请尽可能改善可投放性。 请参阅Adobe Campaign [可投放性](../../sending/using/about-deliverability.md) 详细文档，并联系Adobe Campaign管理员寻求帮助。

* **[!UICONTROL Deliveries with long start pending]**：这通常意味着MTA（消息传输代理）级别出现问题。 执行过程正在等待某些资源的可用性。 MTA可能尚未启动。

  **[!UICONTROL Deliveries with low throughput]**：同样地，这是一个可投放性问题，这意味着MTA过于缓慢。
