...
请参阅上面各种PDU](../../administration/using/sms-protocol.md#information-pdu)部分中的[信息以了解`BIND PDU`中可用的字段，通常是在`address_range`中添加短代码，但这需要提供程序的特别支持。 联系他们，了解他们希望如何独立路由多个短代码。
...
* 检查Adobe Campaign是否及时回复并成功回复了`DELIVER_SM_RESP PDU`。 在Adobe Campaign Standard上，这将保证已应用整个处理逻辑，如果不是这种情况，则保证日志中会显示一条错误消息，说明处理失败的原因。
...
* 如果出现`DELIVER_SM PDU`，请检查Adobe Campaign是否确认它并成功`DELIVER_SM_RESP PDU`（代码0）。 此RESP可确保Adobe Campaign已应用所有处理逻辑（自动回复和允许/）。 如果不是这种情况，请在MTA日志中搜索错误消息。
