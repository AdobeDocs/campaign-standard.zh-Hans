...
1. 如果要创建或编辑基于配置文件的事件，请在部 **[!UICONTROL Targeting enrichment]** 分中选择丰富内容，以在交付执行过程中用作消息目标。

   ![](assets/message-center_marketing_targeting_enrichment.png)

   >[!NOTE]
   >
   >对于基于配置文件的事件，必 **[!UICONTROL Profile]** 须根据资源选择定位扩充。

发布活动和消息后，此链接将允许您丰富事务消息的内容。
...
### 删除活动 {#deleting-an-event}

在取消发布活动或活动尚未发布后，您可以从活动配置列表中删除该活动。 操作步骤：
...
1. 将鼠标悬停在您选择的事件配置上，然后选择按 **[!UICONTROL Delete element]** 钮。
   ...
   >[!NOTE]
   确保活动配置具有状 **[!UICONTROL Draft]** 态，否则您将无法删除它。 状 **[!UICONTROL Draft]** 态适用于尚未发布或已取消发布的活 [动](#unpublishing-an-event)。
...
>[!IMPORTANT]
删除已发布且已使用的事件配置也会删除相应的事务消息及其发送和跟踪日志。
