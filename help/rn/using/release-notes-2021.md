...
* 现在，验证和陷阱轮廓已从动态报告中的投放性能计算中排除。(CAMP-47338)
...
* 设计登陆页面时，您现在可以添加一个必填复选框，要求在提交表单前选择轮廓。有关更多信息，请参阅[详细文档](../../channels/using/managing-landing-page-form-data.md#agreement-checkbox)。
...
* 现在，默认情况下禁用两步轮廓删除流程（从 Campaign 19.4 版本开始被弃用）。以前，必须先从 Campaign 界面手动禁用该功能，然后才能使用隐私核心服务。否则，会导致删除请求在未完成的情况下保持挂起状态。

* 在动态报告中，****“排除校样”区段已被移除。(CAMP-46161)
...
* 现在，通过&#x200B;**轮廓**&#x200B;列表可根据以下字段之一搜索记录：在扩展轮廓资源时，已在高级筛选中添加了电子邮件、名字、姓氏或自定义字段。此功能在使用 filterType 参数的 Campaign Standard API 中也可用。[了解详情](../../audiences/using/integrated-customer-profile.md)
...
* 在丰富事务性消息内容时，当从轮廓表获取数据时，不再检索链接，这会减少消息准备期间的延迟，并避免由于与轮廓表定义的关系不正确而导致轮廓数据为空。
...
* 测试登陆页时用于选择轮廓的的选取器现在将使用 profilBase 资源而不是轮廓，以防止超时。
...
* 事务性短信消息有效日期现在可以由事务性消息 API 中为有效期限参数设置的值定义。(CAMP-36600)
...
* 修复了在发送针对轮廓的事务性推送通知时导致投放分析失败的问题，方法是引入新的[目标映射](../../administration/using/target-mappings-in-campaign.md)：**轮廓 - 推送的实时事件** (*mapRtEventAppSubRcp*)。[基于轮廓的事务性推送通知](../../channels/using/transactional-push-notifications.md#transactional-push-notifications-targeting-a-profile)的投放、排除和跟踪日志现在将存储在 *broadLogAppSubRcp*、*excludeLogAppSubRcp* 和 *trackingLogAppSubRcp* 表中。

  >[!IMPORTANT]
  >
  >由于此更改，如果您使用的是现有的基于轮廓的事务性推送通知（在升级到 Adobe Campaign 21.1 之前创建），建议您将目标映射更新为新目标映射，然后再次发布消息。请参见[此处](../../channels/using/transactional-push-notifications.md#change-target-mapping)的详细步骤。使用上一个目标映射“轮廓 - 实时事件”**** (*mapRtEventRcp*) 可能会导致更长的投放准备时间和性能下降。

* 修复了在显示 5000 行时阻止运行投放报告的问题。
* 修复了在修改投放模板后阻止更新变体 B 的内容的 A/B 测试问题。(CAMP-45235)
* 修复了导致事务性消息传递过程卡住、从而阻止发送消息的问题。
* 修复了在单击内部链接后（例如从校样摘要屏幕访问上层投放时）可能导致导航问题的问题。
...
* 修复了可能阻止显示轮廓的特定详细视图的问题。如果轮廓资源已通过启用了&#x200B;**添加个性化字段部分**&#x200B;选项的自定义字段进行扩展，则会发生此情况。
* 修复了在发送事务事件时可能返回错误代码 500 的问题。(CAMP-46811)
* 修复了可能阻止您接收电子邮件计划报告的问题。(CAMP-46891)
* 修复了在使用 1 基数简单链接将自定义资源链接到轮廓资源链接时发生的问题。当访问自定义资源字段为空的轮廓时，现在会显示错误消息，而不是空列表。
* 修复了在工作流（选择要替换的轮廓时页面无法加载投放轮廓）中使用轮廓替换时的问题。(CAMP-46522)
