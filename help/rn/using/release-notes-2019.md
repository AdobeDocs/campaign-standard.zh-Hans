 ...
 <tbody> 
  <tr> 
   ...
   <ul>
    ...
    <li>“配置文件”资源中已添加<strong>CCPA选择退出</strong>字段，以允许Adobe Campaign用户跟踪消费者是否选择退出了出售个人信息。 <a href="https://helpx.adobe.com/cn/campaign/kb/acs-privacy.html#ccpa">了解详情</a>。</li>
  </ul>
...
* 更新了动态报告的同意弹出窗口，以包含Adobe Campaign Standard和Microsoft Dynamics 365集成。 通过接受条款，在使用Adobe Campaign Standard/Microsoft Dynamics 365集成和动态报告时，将包含用户档案数据。 [阅读更多](../../reporting/using/about-dynamic-reports.md#dynamic-reporting-usage-agreement) (CAMP-29766)
* 修复了在接收投放警报时显示错误联系日期的问题。
* 现在，在使用未知上下文参数提交事务型消息事件时，Campaign会返回“400”错误消息，而不是“500”。 (CAMP-28632)
* 在动态报告中添加了新的&#x200B;**Exclude proof**&#x200B;区段。 现在，默认情况下会选择此区段来过滤您的报表。 [阅读更多](../../reporting/using/list-of-components-.md#segments)
* **消息过期**&#x200B;选项已添加到推送通知中。 利用该功能，可指定Apple(APNS)或Android(FCM)将不再发送消息的过期日期。 [阅读更多](../../channels/using/customizing-a-push-notification.md#add-expiration-date)
* **加载文件**&#x200B;活动已得到改进：工作流日志已变得更清晰，并更详细地说明了在文件加载失败时发生的错误。 激活&#x200B;**在文件**&#x200B;中保留rejects选项时生成的叫客过渡已重命名为&#x200B;**Rejects**。 [阅读更多](../../automating/using/load-file.md)
...
* 现在，使用Email Designer编辑链接时，可以使用&#x200B;**下划线链接**&#x200B;选项。 此外，还添加了&#x200B;**Target**&#x200B;属性，其默认值设置为&#x200B;**None**。 [阅读更多](../../designing/using/styles.md#about-styling-links)
* 修复了电子邮件正文文本组件中的链接的颜色问题。 (CAMP-37330)
* 修复了删除图像时无法删除关联链接的问题。 (CAMP-37234)
* 修复了无法在条件中对动态内容的&#x200B;**Order**&#x200B;设置进行保存修改的问题。 (CAMP-36883)
* 修复了搜索登陆页面时的问题。 搜索已从最初创建的50个扩展到所有数据库。 (CAMP-36839)
* 修复了在&#x200B;**发件人中保存对电子邮件发件人所做的修改时的问题：名称**&#x200B;字段。 (CAMP-36606)
* 已修改轮播组件兼容性警告，以反映支持的电子邮件客户端。
* 修复了移动设备上的显示问题。 现在，高度属性始终设置为“高度：自动”。 (CAMP-35497)
* 修复了从结构组件中删除片段时HTML中的左侧样式和元标记的问题。 (CAMP-35390)
...
* 现在，可以将&#x200B;**Load file**&#x200B;活动中的&#x200B;**Rejects file**&#x200B;的名称个性化，其名称与&#x200B;**File export**&#x200B;活动中的名称相同。
* 现在，可以通过API访问所有未链接到任何现成实体的自定义实体。
* 提高了大型资源上的数据库性能。
* 更清晰地描述了发送短信消息时出现的一些错误。 (CAMP-36558)
* 现在，在执行直接或通过多个活动连接到自身的工作流的&#x200B;**调度程序**&#x200B;活动时，会显示一条错误消息，因为这可能导致实例的工作流服务器卡住。
...
* 修复了新扩展的&#x200B;**Profiles**&#x200B;自定义资源无法用于查询的问题。 (CAMP-35119)
* 修复了激活了共享配置的实例的&#x200B;**修复数据库结构**&#x200B;模式。 (CAMP-35118)
* 修复了在broadlogs中添加聚合数据时导致SQL日志错误的问题。 (CAMP-35034)
* 修复了在创建&#x200B;**Segmentation**&#x200B;活动时过渡的问题。 (CAMP-35033)
* 修复了&#x200B;**Query**&#x200B;活动中阻止&#x200B;**encryption_aescbcDecrypt**&#x200B;函数解密&#x200B;**encryption_aescbcEncrypt**&#x200B;函数的问题。 (CAMP-34952)
* 修复了可能阻止在投放中显示&#x200B;**跟踪日志**&#x200B;的问题。 (CAMP-34855)
* 修复了使用&#x200B;**发送时间优化**&#x200B;自定义日期公式时的问题，该问题可能会由于工作流的附加数据出错而阻止发送推送通知。 (CAMP-30336)
* 修复了可能阻止发布自定义资源的问题。 (CAMP-37425)
* 修复了阻止管理员用户修改导入包的问题。  (CAMP-37176)
* 修复了在投放活动连接到空&#x200B;**读取受众**&#x200B;活动时，工作流中阻止发送校样的问题。 (CAMP-37164)
...
在19.2版本中引入了一系列改进和修复，以优化片段。 新创建的片段将可以无缝工作。 之前构建的片段呈灰显状态，需要迁移到新格式。 为此，请单击每个片段并验证其迁移到新格式。 我们建议您在迁移一些片段之前先测试它们。
...
<table> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> <p>在成功满足定义的条件时，通过另一个工作流中的有效负载或REST API调用启动工作流，以与外部系统集成。 此外，还包含一个新的<strong>test</strong>活动，您可以在该活动中对此功能运行测试。</p><p>有关更多信息，请参阅<a href="../../automating/using/calling-a-workflow-with-external-parameters.md">详细文档</a>和<a href="https://experienceleague.adobe.com/docs/campaign-learn/campaign-standard-tutorials/managing-processes-and-data/execution-activities/external-signal-activity.html">操作方法视频</a>。</p></td> 
  </tr> 
  <tr> 
...
* 修复了在删除Email Designer HTML模式后仍在其中显示Microsoft Office标记的问题。 (CAMP-32141)
...
* 修复了在HTML模式下创建列表或按钮等元素时，无法在纯文本模式下正确显示的问题。 （CAMP-32582 和 CAMP-32542）
...
* 修复了使用&#x200B;**定位移动应用程序**&#x200B;模板的所有用户时阻止个性化字段工作的错误。
...
* 修复了在Adobe Experience Manager中修改主题后单击&#x200B;**刷新AEM内容**&#x200B;选项时，主题行无法更新的问题。 (CAMP-29984)
* 修复了阻止从Adobe Target使用动态图像的几个问题。
* 修复了在准备时检索内容时，如果先前从URL导入了内容，则预览无法更新的问题。
* 已将YouTube图标添加到&#x200B;**Social**&#x200B;内容组件。
* 为Email Designer面板中显示的内容组件和片段添加了&#x200B;**列表**&#x200B;视图。

**其他改进**

* Adobe Campaign现在完全兼容SDK V4和AEP SDK应用程序。
* Adobe Campaign支持Wear OS by Android以及Apple的watchOS推送通知。
 ...
 <tbody> 
  <tr> 
   ...
   <td> <p>新的直观Email Designer（以前称为Creative Designer）已移至GA。 现在，它支持旧版内容编辑器中的所有功能，包括：</p> 
    <ul> 
     <li> 使用来自Adobe Target的<a href="../../integrating/using/adding-target-dynamic-content.md">动态图像</a> </li> 
     <li> 能够在准备时从URL中自动检索内容</a><a href="../../designing/using/using-existing-content.md#retrieving-content-from-a-url-automatically-at-preparation-time"> </a></li> 
     <li> 完全兼容的<a href="../../designing/using/using-reusable-content.md#content-templates">现成内容模板</a>。 </li> 
    </ul> 
    <p>有关更多信息，请参阅<a href="../../designing/using/designing-content-in-adobe-campaign.md">详细文档</a>和<a href="https://experienceleague.adobe.com/docs/campaign-learn/campaign-standard-tutorials/designing-content/email-designer/email-designer-overview.html">操作方法视频</a>。下面列出了改进和修复。</p><p>因此，现已弃用旧版电子邮件内容编辑器。 有关详细信息，请参见此<a href="https://experienceleague.adobe.com/docs/campaign-standard/using/release-notes/deprecated-features.html">页面</a>。</p> </td> 
  </tr> 
  <tr> 
...
* 修复了Email Designer中应用于按钮的样式设置的Microsoft Outlook呈现失真的问题。
* 修复了Email Designer中的一个问题，该问题导致可以从电子邮件中使用的内容片段中编辑URL，该问题不是预期的，因为该片段默认处于锁定状态。
* 修复了Email Designer分隔符组件无法在Microsoft Office中显示的问题。
...
* 修复了营销活动和资产核心服务集成的问题，该问题可能会在电子邮件中选择&#x200B;**[!UICONTROL Image shared from Adobe Experience Cloud]**&#x200B;时阻止显示资产(CAMP-28732)。
