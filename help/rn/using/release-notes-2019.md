...
* **消息过期**&#x200B;选项已添加到推送通知中。 它允许您指定到Apple (APNS)或Android (FCM)不再发送消息的截止日期。 [了解更多信息](../../channels/using/customizing-a-push-notification.md#add-expiration-date)
* 已对&#x200B;**加载文件**&#x200B;活动进行改进：工作流日志已更清晰、更详细地记录文件加载失败时发生的错误。 激活&#x200B;**将拒绝保留在文件中**&#x200B;选项时生成的叫客过渡已重命名为&#x200B;**拒绝**。 [了解更多信息](../../automating/using/load-file.md)
...
* 使用电子邮件Designer编辑链接时，您现在可以使用&#x200B;**Underline链接**&#x200B;选项。 此外，已添加&#x200B;**Target**&#x200B;属性，其默认值设置为&#x200B;**无**。 [了解更多信息](../../designing/using/styles.md#about-styling-links)
* 修复了电子邮件正文中文本组件中链接的颜色问题。 (CAMP-37330)
* 修复了在删除图像时阻止删除关联链接的问题。 (CAMP-37234)
* 修复了一个问题，该问题阻止在条件中保存对动态内容的&#x200B;**订单**&#x200B;设置所做的修改。 (CAMP-36883)
...
* 现在，**加载文件**&#x200B;活动中的&#x200B;**拒绝文件**&#x200B;的名称可以在与&#x200B;**文件导出**&#x200B;活动相同的内容中进行个性化。
...
* 修复了新扩展的&#x200B;**用户档案**&#x200B;自定义资源不可用于查询的问题。 (CAMP-35119)
* 修复了激活分片配置的实例的&#x200B;**修复数据库结构**&#x200B;模式。 (CAMP-35118)
* 修复了在broadlog上添加聚合数据时导致出现SQL日志错误的问题。 (CAMP-35034)
* 修复了在创建&#x200B;**分段**&#x200B;活动时过渡的问题。 (CAMP-35033)
* 修复了&#x200B;**查询**&#x200B;活动中导致&#x200B;**encryption_aescbcDecrypt**&#x200B;函数无法解密&#x200B;**encryption_aescbcEncrypt**&#x200B;函数的问题。 (CAMP-34952)
* 修复了可能导致投放中无法显示&#x200B;**跟踪日志**&#x200B;的问题。 (CAMP-34855)
* 修复了在使用&#x200B;**发送时间优化**&#x200B;自定义日期公式时的问题，该公式可能会由于工作流的附加数据有错误而阻止发送推送通知。 (CAMP-30336)
* 修复了可能导致自定义资源无法发布的问题。 (CAMP-37425)
* 修复了阻止管理员用户修改导入包的问题。  (CAMP-37176)
* 修复了工作流中阻止发送校样的问题（如果投放活动连接到空的&#x200B;**读取受众**&#x200B;活动）。 (CAMP-37164)
...
* 从“电子邮件Designer设置”窗格中删除了一些不需要的字段。
...
* 现在，在创建没有聚合期的定期投放时，可通过投放仪表板在发送投放之前请求确认。 有关详细信息，请参阅[详细文档](../../sending/using/confirming-the-send.md)。
* 您现在可以使用在工作流外部信号活动中声明的事件变量将投放标签个性化。 有关详细信息，请参阅[详细文档](../../automating/using/calling-a-workflow-with-external-parameters.md)。
* GDPR删除查询已得到改进，以实现更好的性能。 (CAMP-33504)
* “ftp”选项已从外部帐户配置界面中删除。 (CAMP-34472)
* 现在，您可以为每个电子邮件启用和禁用SMTP测试模式选项。 有关详细信息，请参阅[详细文档](../../administration/using/configuring-email-channel.md#smtp-test-mode)。 (CAMP-34602)
...
* 修复了在接收使用Outlook上的Email Designer创建的电子邮件时视区背景颜色出现的问题。 (CAMP-31402)
* 修复了可能导致使用电子邮件Designer创建的电子邮件内容在Outlook中打开时无法响应的问题。 (CAMP-31400)
...
* 修复了在导入包含事件配置和关联的事务型消息的包时发生的问题。 有关详细信息，请参阅[详细文档](../../channels/using/getting-started-with-transactional-msg.md#exporting-and-importing-transactional-messages)。
* 修复了从为包含产品清单的事务型消息创建的测试用户档案中删除集合数据的问题。

**其他变更**

* 短信外部帐户中添加了一个新选项。 它允许限制发送短信的MTA进程的最大数量，以便更好地控制并行连接的数量。 有关详细信息，请参阅[SMS连接器协议和设置](https://helpx.adobe.com/cn/campaign/kb/sms-connector-protocol-and-settings.html)技术说明。
* 现在，使用API扩展发布资源时，如果该API已发布，那么在每次再次发布它时，都会自动更新该API。 以前，此操作是手动执行的，且未能更新API可能会破坏此API的配置文件或服务资源。 有关详细信息，请参阅[详细文档](../../developing/using/updating-the-database-structure.md#publishing-a-resource-with-api-extension)。
...
<table> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> <p>此发行版本包含了Adobe Campaign与Adobe Experience Platform Launch和Mobile SDK中适用于Adobe Campaign Standard的Android和iOS扩展的GA版本的集成。 这些扩展支持推送消息、应用程序内消息传送和移动应用程序配置文件更新。</p><p> 有关更多信息，请参阅<a href="https://helpx.adobe.com/cn/campaign/kb/configuring-app-sdk.html">详细文档</a>。</p> </td> 
  </tr> 
  <tr> 
...
* 修复了一个问题，在Adobe Experience Manager中修改主题后，单击&#x200B;**刷新AEM内容**&#x200B;选项时，该问题阻止更新主题行。 (CAMP-29984)
* 修复了阻止使用Adobe Target中的动态图像的几个问题。
* 修复了在准备时检索内容时，如果内容先前是从URL导入的，则无法更新预览的问题。
* YouTube图标已添加到&#x200B;**Social**&#x200B;内容组件。
* 已为“电子邮件Designer”调色板中显示的内容组件和片段添加&#x200B;**列表**&#x200B;视图。
...
<table> 
 ...
 <tbody> 
  ...
  <tr> 
   <td> 应用程序内消息传送Beta改进<br /> </td> 
   <td> <p>应用程序内消息传送Beta功能已得到增强，现可提供以下功能：</p> 
    <ul> 
     <li> 应用程序内Beta渠道符合GDPR </li> 
...
* 现在，可以使用电子邮件Designer创建用户档案事务型消息。 (CAMP-28900)
* 修复了Email Designer中的错误，该错误会导致在准备期间从URL自动检索内容时内容可编辑，而此错误应被锁定。

**修补程序**

* 修复了在动态报告中显示错误投放日志的问题。 (CAMP-23446)
* 修复了可能影响退回摘要报表中数字的问题(CAMP-28703)
* 修复了Campaign与Assets核心服务集成的问题，该问题可能导致在电子邮件中选择&#x200B;**[!UICONTROL Image shared from Adobe Experience Cloud]**&#x200B;时无法显示资源(CAMP-28732)。
