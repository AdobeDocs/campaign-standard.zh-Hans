...
Adobe通过添加新功能、增强功能和修复，不断改进其解决方案。

所有 Adobe Campaign Standard 实例都会随每次版本更新一起升级。升级不需要任何操作。

升级分两个阶段进行。首先，升级Stage实例，以便测试新功能并根据需要调整配置。 然后，将升级生产实例。

所有发行日期都可能发生更改：请定期访问此页面以检查更新。
...
答：不会。有几分钟不会发送消息。升级完成后，进程将自动重新启动。

**问：工作流是否会继续运行并发送投放？**

答：不会。在版本升级的过程中，工作流服务器和 MTA 都会停止。因此，在几分钟内不会运行工作流，也不会发送投放。 无需执行任何操作：一旦实例升级完成，工作流将立即重新启动。
