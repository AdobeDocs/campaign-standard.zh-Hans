...
* 登陆页面不会自动显示记录列表，不会列出轮廓已订阅的服务。 如需有关服务的更多信息，请参阅[此章节](../../audiences/using/creating-a-service.md)。

* 带有预填表单（数据随页面预加载）的登陆页面，只能通过 Adobe Campaign 电子邮件访问。 无法从网站页面访问此类表单。

**协调**

* 协调行为如下：一旦找到匹配项，协调进程就会停止。 这意味着，存在重复项时，只能对一个轮廓记录进行协调，而不能对多个记录进行协调。

例如，您希望向轮廓发送以下客户获取登陆页面，以便使用轮廓的移动号码更新 Campaign 数据库。

![](assets/landing_page_limitation_1.png)

如果其中某个轮廓用新信息填充登陆页，但已存在重复的轮廓，则将更新为创建日期较早的匹配轮廓，因为轮廓的优先级取决于其创建日期。

此处只更新了第一个轮廓，因为它是最早的条目。

![](assets/landing_page_limitation_2.png)

**测试登陆页面**

* 登陆页面只能用于轮廓，而不能用于测试轮廓，这意味着登陆页面不能通过电子邮件验证进行测试。
...
您还可以配置确认按钮在被轮廓单击后的行为方式，并根据您的需求（图像、个性化字段等）将内容个性化。 例如，您可以在登录页的确认页面上插入轮廓的名字，以感谢他们进行注册。
...
>[!IMPORTANT]
>
>登陆页面测试只能使用轮廓进行，不能使用测试轮廓进行。 提交表单后，所选轮廓的数据将实时更新。 要避免修改真实的轮廓，请使用虚假客户的轮廓。
...
发布后，您的登陆页面即可供使用。 然后，您可以设置不同的机制，以便访问登陆页面从而将新的轮廓加入数据库，或获取关于现有轮廓的其他信息。
...
>[!VIDEO](https://video.tv.adobe.com/v/35533?captions=chi_hans&quality=12)
